...
/**
 * Mahony algoritmi asennon tunnistamiseen.
 */
public class MahonyAHRS
{
    private float DEFAULT_SAMPLE_FREQ = 512.0f;	// sample frequency in Hz
    ...
    public float[] ComputeAngle()
    {
        ...
        result[1] = (float) Math.toDegrees(Math.atan2(q0*q1 + q2*q3, 0.5f - q1*q1 - q2*q2));
        result[0] = (float) Math.toDegrees(Math.asin(-2.0f * (q1*q3 - q0*q2)));
        result[2] = (float) Math.toDegrees(Math.atan2(q1*q2 + q0*q3, 0.5f - q2*q2 - q3*q3));
        return result;
    }
